...
WZ analysis searches for the presence of both a W and a Z boson decaying into three leptons. This process is interesting because it is a probe for triple gauge couplings. To search for the process, the following criteria were used: 

- There are exactly three good leptons with pT > 25 GeV;
- WZ candidate is chosen by finding the Z boson candidate closest to the nominal Z
mass
- |mass lepton pair - mass Z | < 10 GeV
- Reconstructed transverse mass W > 30 GeV.

For more details on how these criteria were implemented, see the WZ section of code in the appendix. 
In the WZ analysis, the W boson produces a lepton and a neutrino and the Z boson produces two leptons. The mass of the Z boson can therefore be inferred from the sum of the mass of the two leptons identified as the ones that came from the Z boson, and the W mass can be reconstructed using equation 2.
The results clearly show that the invariant mass graph peaks at around the 90 GeV bin, which corresponds to the mass of a Z boson which is measured to be 91.1876 /citep{beringer et al. _2012}. The reconstructed mass graph peaks at the 80 GeV bin, which also corresponds to the mass of a W boson which was measured to be 80.385 /citep{beringer et al. _2013}. Therefore, the analysis clearly selects the correct decay mode. 
Out of all the run 1 data obtained from CERN, only 857 events satisfied the selection criteria. This low value agrees with the 
...
%% DHRUVA BEGIN
Most of the code produced are modifications to the original treelooper.cxx file, which runs the analysis. As such, most of the body of code is repeated among different analyses, with the only changes being within an iterative loop that runs over the data set. For the sake of brevity, we will present the body of code without the for loop in this section, and only the for loops for the analyses on the following pages.
...
for(Long64_t jentry=0; jentry<nentries; jentry++) { // begin event loop
		...
		//Reject events that do not pass the GRL (important for data) or that don't have a good reconstructed vertex
        ...
        //The following three if statements go through each combinations of two 
        oppositely charged leptons to see which has an invariant mass closest to
        the Z mass. The two leptons with the closest invariant mass are then 
        assumed to have been produced by the Z boson, and the remaining lepton
        is assumed to be have been produced by the W boson.
			mlp=0.0; 
